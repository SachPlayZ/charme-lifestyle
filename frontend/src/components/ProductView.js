...
const ProductDetails = () => {
  ...
  useEffect(() => {
    async function fetchProductDetails() {
      try {
        const response = await fetch(`http://ec2-13-126-58-192.ap-south-1.compute.amazonaws.com/products?id=${id}`, {
          method: 'GET',
        ...
        {product ? (
          ...
          <div className="header">
            <div className="header-logo">
              <Link to="/marketplace"><span className='whitetext'>Charm</span><span className='pink'>è</span></Link>
            </div>
            
          </div>
          <div className="product">
            <div className="product-photo">
              <img src={'http://ec2-13-126-58-192.ap-south-1.compute.amazonaws.com/products/image?id=' + product.id + '&n=1'} alt={product.name} />
              <img src={'http://ec2-13-126-58-192.ap-south-1.compute.amazonaws.com/products/image?id=' + product.id + '&n=2'} alt={product.name} />
            </div>
