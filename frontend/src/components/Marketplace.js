...
const getData = async () => {
  try {
    const response = await fetch(
      "http://ec2-13-126-58-192.ap-south-1.compute.amazonaws.com/products",
      {
        method: "GET",
              ...
              data1.products.map((getProd, index) => (
                <div key={index} className="col pb-3">
                  <div className="card ms-auto me-auto">
                    <img
                      src={`http://ec2-13-126-58-192.ap-south-1.compute.amazonaws.com/products/image?id=${getProd.id}&n=${hoveredProduct === getProd.id ? 2 : 1}`}
